...
try:
    from .clientui import (
        clear_screen,
        format_owned_territories,
        get_player_snapshot,
        print_phase_header,
        print_player_status,
            print_player_cards,
            print_waiting_summary,
        prompt_card_indices,
        prompt_choice,
        prompt_positive_int,
        prompt_territory,
    ...
    while True:
        ...
        if current_phase == GameState.FINISHED:
            ...
            if victory.get("game_over"):
                ...
                print("No winner information available.")

            print()
            print("Type 1 to start a new match with the same players, or 2 to exit.")
            choice = input("Choice: ").strip()
            if choice == "1":
                result = game.start_new_match()
                if "error" in result:
                    print(f"Could not start a new match: {result['error']}")
                    print("Exiting client.")
                    break
                else:
                    print("New match started. Waiting for setup...")
                    time.sleep(1)
                    # continue the loop to follow new game
                    continue
            else:
                break
        ...
        if turn_id != my_id:
            ...
            if not turn_waited_message:
                # show a prettier waiting summary
